...
source "$HOME/.local/bin/env"  # 由 boris 添加.
...
torchrun --standalone --nproc_per_node=2 -m scripts.mid_train -- --run=$WANDB_RUN --device_batch_size=4
...
torchrun --standalone --nproc_per_node=2 -m scripts.chat_sft -- --run=$WANDB_RUN --device_batch_size=4
